...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	WriteBuffer[0] = (reg << 1) | 0x02;  //DIR = 1 for write
	WriteBuffer[1] = val;

	Status = XSpi_SetSlaveSelect(&SpiInstance, ~2); //slave select
	if (Status != XST_SUCCESS) {
		xil_printf("MAXreg_wr: Slave select failed\n");
		return;
	}

	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2); //perform transfer
	if (Status != XST_SUCCESS) {
		xil_printf("MAXreg_wr: SPI transfer failed, status:: %d\n", Status);
	}
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	BYTE WriteBuffer[257];  //1 command byte max + 256 data bytes
	BYTE ReadBuffer[257];
	...
	//ss
	Status = XSpi_SetSlaveSelect(&SpiInstance, ~2);
	if (Status != XST_SUCCESS) {
		xil_printf("MAXbytes_wr: Slave select failed\n");
		return data;
	}

	//transfer: 1 command byte + n bytes data
	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes + 1);
	if (Status != XST_SUCCESS) {
		xil_printf("MAXbytes_wr: SPI transfer failed, status: %d\n", Status);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return val
	BYTE WriteBuffer[2] = {0};
	BYTE ReadBuffer[2] = {0};

	//prep command byte: register address shifted left by 1, read bit clear
	WriteBuffer[0] = (reg << 1) | 0x00;  //DIR = 0 for read
	WriteBuffer[1] = 0x00;  //dummy byte to clock in response

	Status = XSpi_SetSlaveSelect(&SpiInstance, ~2);
	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);
	u32 timeout = 0x10000;
	volatile u32 *spi_status_reg = (u32 *)(XPAR_SPI_USB_BASEADDR + 0x64);  // SPISR at offset 0x64

	while (((*spi_status_reg & 0x04) == 0) && timeout--) {  // Bit 2 = RxFull in FIFO mode
		// Wait
	}

	if (timeout == 0) {
		xil_printf("SPI transfer timeout! Status=0x%08X\n", *spi_status_reg);
		return 0xFF;
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	BYTE WriteBuffer[257];  //1 command byte + 256 dummy bytes
	BYTE ReadBuffer[257];

	//prep command byte
	WriteBuffer[0] = (reg << 1) | 0x00;  //DIR = 0 for read

	//dummy bytes to clock in the data
	for (int i = 0; i < nbytes; i++) {
	...
	Status = XSpi_SetSlaveSelect(&SpiInstance, ~2);
	if (Status != XST_SUCCESS) {
		xil_printf("MAXbytes_rd: Slave select failed\n");
		return data;
	}
	...
	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes + 1);
	if (Status != XST_SUCCESS) {
		xil_printf("MAXbytes_rd: SPI transfer failed with status %d\n", Status);
		return data;
	}

	//copy received data (skip first/status byte
	for (int i = 0; i < nbytes; i++) {
